...
from Model_component.Ladan_component_Criminal import Ladan_Criminal
from Model_component.GraphRecorder import GraphRecorder
...
class Ladan_criminal(Model):
    ...
    def build_components(self):
        ...
        with tf.name_scope('define_model'):
            ...
            with tf.name_scope('define_FeatureExtracter'):
                self.LadanPPK = Ladan_Criminal(config=self.config, group_num=self.group_num,
                                               trainable=self.trainable, name='Ladan_model')

                self.PosteriorGraph = GraphRecorder(node_num=self.accu_num, feature_dim=256, name='graph_posterior',
                                                    trainable=False)
    ...
    def call(self, inputs, training=None, warming_up=True, triplet=False, dropout=None, **kwargs):
        fact_input = inputs
        ...
        with tf.name_scope('model_process'):
            ...
            accu_posterior, accu_adj_matrix_posterior = self.PosteriorGraph(fact_description)

            fact_rep, group_pred_prior = \
                self.LadanPPK(fact_description, training=training, dropout=dropout,
                              law_information=[law_description, self.law_adj_matrix, self.group_indexes, accu_posterior,
            ...
            output_accu = self.Accu_Scalar(self.Accu_decoder(accu_hidden))
    ...
    def test_step(self, data):
        ...
        y_pred = self(x, training=False, warming_up=False, dropout=None)
    ...
    def predict_step(self, data):
        ...
        return self(x, training=False, warming_up=False, dropout=None)
    ...
    def fit_warmingup(self, x=None, y=None, batch_size=None, epochs=1, verbose=1, callbacks=None,
                      ...
                      ave_acc=True, triplet_data=None, **kwargs):
                ...
                with data_handler.catch_stop_iteration():
                    for step in data_handler.steps():
                        with trace.Trace(
                            ...
                            tmp_logs = train_function(iterator, warming_up=self.warming_up, triplet=False)
