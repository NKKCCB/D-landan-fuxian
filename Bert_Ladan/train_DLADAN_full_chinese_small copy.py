...
def load_dataset(group_indexes):
    train_path = '/data1/hhc/datasets/processed_dataset/CAIL_new/full_doc/small/train_bert_chinese.pkl'
    valid_path = '/data1/hhc/datasets/processed_dataset/CAIL_new/full_doc/small/valid_bert_chinese.pkl'
    test_path = '/data1/hhc/datasets/processed_dataset/CAIL_new/full_doc/small/test_bert_chinese.pkl'
    
    train_Dataset = DladanCailLoader(data_path=train_path, group_indexes=group_indexes, mode='train')
    valid_Dataset = DladanCailLoader(data_path=valid_path, group_indexes=group_indexes, mode='valid')
    ...
    train_dataset = DataLoader(dataset=train_Dataset, batch_size=56,
    ...
    valid_dataset = DataLoader(dataset=valid_Dataset, batch_size=112,
                               num_workers=1, drop_last=False,
                               shuffle=False, collate_fn=collate_fn,
                               # sampler=RandomSampler(Dataset)
                               )
    
    test_dataset = DataLoader(dataset=test_Dataset, batch_size=112,
                               num_workers=1, drop_last=False,
                               shuffle=False, collate_fn=collate_fn,
                               # sampler=RandomSampler(Dataset)
                               )
...
def train_loop(model: DLADAN_Bert_full, train_dataset, valid_dataset, test_dataset, 
    ...
    for epoch_num in range(epoches):
        ...
        model.eval()
        final_f1 = valid_loop(model, test_dataset, mode="Test")
...
if __name__ == "__main__":
    ...
    # 1. load dataset
    config_file= '/data1/hhc/code/D-LADAN-main/Bert_Ladan/Config/DLADAN_Bert_config.json'
    with open(config_file) as f:
        config = json.load(f, object_hook=lambda d: SimpleNamespace(**d))
    warming_up_epoch = 1
        
    law_inputs, group_list, graph_membership, law_adj_matrix = \
        get_law_graph_adj(threshold=0.35, sent_len=config.train.law_sentence_len,
                          pretrained_bert_fold=config.train.pretrain_model_path_1)
    group_indexes = list(zip(*graph_membership))[1]
    group_num = len(group_list)
    train_dataset, valid_dataset, test_dataset = load_dataset(group_indexes)
    
    # 2. define model 
    model:DLADAN_Bert_full = \
        DLADAN_Bert_full(config=config, group_num=group_num, group_indexes=group_indexes,
                         law_input=law_inputs, law_adj_matrix=law_adj_matrix, accu_relation=True, mode='small')
    ...
    scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.8, min_lr=1e-6, patience=2)
    epoches = config.train.epoch
    
    # 3. Training loop
    model_save_path = '/data1/hhc/code/D-LADAN-main/Bert_Ladan/saves/CAIL_small/DLADAN_BERT_full_chinese.hdf5'
    train_loop(model=model, train_dataset=train_dataset, valid_dataset=valid_dataset, test_dataset=test_dataset,
               epoches=epoches, scheduler=scheduler, model_save_path=model_save_path,
               warming_up_epoches=3, momentum_steps=2)
