...
def get_cutter(dict_path="/data1/hhc/code/D-LADAN-main/law_processed/Thuocl_seg.txt", mode='thulac', stop_words_filtered=False):
    if stop_words_filtered:
        stopwords = stopwordslist('/data1/hhc/code/D-LADAN-main/law_processed/stop_word.txt')  # 这里加载停用词的路径
    else:
        stopwords = []
    if mode == 'jieba':
...
def gen_law_relation(tokenizer, law_label2index_path='/data1/hhc/code/D-LADAN-main/law_processed/law_label2index.pkl', max_length=400):
    law_file_order = pkl.load(open(law_label2index_path, 'rb'))
    n_law = len(law_file_order)
    law_list = law_to_list('/data1/hhc/code/D-LADAN-main/law_processed/criminal_law.txt')
...
def get_law_graph_adj(threshold, sent_len=400, pretrained_bert_fold="/home/nxu/LEVENs/LEVEN-main/LCR_with_LawArticles/pretrain_model/xs/"):
    ...
    neigh_mat, law_index_matrix, law_num = \
        gen_law_relation(tokenlizer, law_label2index_path='/data1/hhc/code/D-LADAN-main/law_processed/law_label2index.pkl',
                         max_length=sent_len)
    zero_matrix = np.zeros_like(neigh_mat, dtype=float)
    one_metrix = np.ones_like(neigh_mat, dtype=float)
    adj_matrix = np.where(neigh_mat > threshold, one_metrix, zero_matrix)
    adj_matrix = adj_matrix - np.eye(N=law_num, dtype=float)
...
def get_accu_graph_adj(tokenizer: BertTokenizer, threshold, word2id_file, id2charge_path, emb_path):
    ...
    zero_matrix = np.zeros_like(cos_sim_matrix, dtype=float)
    one_metrix = np.ones_like(cos_sim_matrix, dtype=float)
    adj_matrix = np.where(cos_sim_matrix > threshold, one_metrix, zero_matrix)
    adj_matrix = adj_matrix - np.eye(N=charge_num, dtype=float)
