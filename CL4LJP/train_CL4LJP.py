...
if __name__ == "__main__":
    ...
    valid_set = read_dataset(data_path, data_version, 'valid')
    ...
    for epoch in range(max_epoch):
        ...
        for step, data in enumerate(tqdm(train_dataset)):
            ...
            charge_loss, article_loss, time_loss, article_CL, charge_CL, label_AL, label_CL = \
                out_dict['charge_loss'], out_dict['article_loss'], out_dict['time_loss'], out_dict['article_CL'], \
                out_dict['charge_CL'], out_dict['label_AL'], out_dict['label_CL']

            loss = charge_loss + article_loss + time_loss + (article_CL + charge_CL + label_AL + label_CL) * 0.5
                   # + (article_CL + charge_CL + label_CL) * 0.5
            tr_loss += loss.item()

            task_loss += (charge_loss + article_loss + time_loss).item()
            a_CL += article_CL.item()
            c_CL += charge_CL.item()
            l_AL += label_AL.item()
            l_CL += label_CL.item()
            loss.backward()
        ...
        print('article_CL:', a_CL / epoch_steps)
        print('charge_CL:', c_CL / epoch_steps)
        print('label_AL:', l_AL / epoch_steps)
